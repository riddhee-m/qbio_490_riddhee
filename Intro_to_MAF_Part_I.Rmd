...
write.csv(clinical, "/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/analysis_data/brca_clinical_data.csv", row.names = FALSE)
```
...
Refer to a codon chart and amino acid abbreviation chart to answer the following:
 - What mutation could cause a S/R change? AGA to AGT
 - What about an A/T change? GCT to ACT
...
Mutation at gene position 728 resulting in Proline to an Alanine. (not sure about "fs*11")
...
What does this p-value tell us? This p-value tells us that the correlation between vital status and breast carcinoma estrogen receptor status is statistically significant.

Does this make sense given the biology of cancer? Yes, as people that are positive for estrogen receptors tend to have a better chance of remission.
...
```{r Exercise 6.3}
num_pats_geneA <- length(geneA_maf@clinical.data$Tumor_Sample_Barcode)
num_pats_geneB <- length(geneB_maf@clinical.data$Tumor_Sample_Barcode)
```
...
Are your mutations co-occurent or mutually exclusive? The mutations are co-occurent.
...
ggsave("/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/week7_maf/oncoplot.png")
```
...
Do you see any clustering between old and young patients? Yes, there is some clustering.
...
coOncoplot(m1 = young_maf, 
           m2 = old_maf, 
           m1Name = 'Young Patients (Age <= 58)', 
           m2Name = 'Old Patients (Age > 58)', 
           borderCol = NA)

ggsave("/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/coOncoplot.png") # <-- saves empty png file
```
...
ggsave("/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/week7_maf/lollipop_plot.png") # <-- saves empty png file
```
...
lollipopPlot2(m1 = young_maf, 
              m2 = old_maf, 
              m1_name = 'Young Patients (Age <= 58)',
              m2_name = 'Old Patients (Age > 58)',
              gene = "TP53") ## pick any gene of your choosing to fill in here
```

Now, interpret your plot. Answer the following questions:

- Does it look like the gene is more commonly mutated in young or old patients? It is commonly mutated young patients.
- Are there more mutations in a specific region of the protein? Why might that be? There are more mutations in the P53 domain.
- What kinds of mutations are the most common? Missense mutations
- Are there any other interesting observations? N/A
...
Provide a brief description of your chosen gene and it's posited relationship to survival. When TP53 mutations are inherited, they cause LFS, a disease that leaves people with a 90 percent chance of developing cancer in their lifetime.

What can you conclude from your plot? TP53 mutation is correlated to survival rate, but this is statistically unsignificant, since the p-value is greater than 0.05.

Is this what you would expect according to the literature? Why or why not? No, since the literature suggests that TP53 mutation leads to higher risk for cancer, so the plot should show that mutants have a lower chance of survival over time than non mutants.

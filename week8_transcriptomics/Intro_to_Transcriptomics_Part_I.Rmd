...
dim(rna_se@colData) #rows: patients columns: clinical data
dim(rna_se@rowRanges@elementMetadata) #rows: genes columns: information about gene
dim(rna_se@assays@data$unstranded) #rows: genes columns: patients
...
```{r Exercise 1.3}
# colData: 
rownames(rna_se@colData) #patient ids
...
```{r Exercise 3.4}
write.csv(rna_genes, "/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/analysis_data/brca_rna_gene_data.csv", row.names = FALSE)
```
...
```{r Exercise 4.4}
write.csv(rna_counts, "/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/analysis_data/brca_rna_count_data.csv", row.names = TRUE)
```
...
summary(rna_clinical$age_at_index) #use this
...
RYR2_mask <- ifelse(rna_genes$gene_name == "RYR2", T, F) # create mask based on the gene name (your mask should be true when the name is 'geneA' and false for any other gene name)
...
```{r Exercise 6.5}
RYR2_ensembl <- rna_genes$gene_id[RYR2_mask]
...
```{r Exercise 7.1}
RYR2_outlier_mask <- ifelse(as.numeric(RYR2_counts) > 500, F, T)
RYR2_counts_cleaned <- RYR2_counts[, RYR2_outlier_mask]

MUC12_counts_cleaned <- MUC12_counts[, RYR2_outlier_mask]
...
plot(as.numeric(RYR2_counts[1,]),
     ...
     xlab = "RYR2 Counts",
     ylab = "MUC12 Counts",
     main = "RYR2 Counts vs MUC12 Counts",
     log = "xy"
)
```

Do you notice any patterns? Is there a relationship between the two genes that you chose? Write a few sentence making observations about your plot, noting the significance of what this means, and any key takeaways: There are no significant patterns between the RYR2 counts and the MUC12 counts. 
...
```{r Exercise 7.2}
geneAB_counts <- data.frame(as.numeric(RYR2_counts_cleaned), as.numeric(MUC12_counts_cleaned)) 
                            # create data frame with geneA_counts and geneB_counts
par(mar=c(1,1,1,1))
boxplot(geneAB_counts,
        xlab = c("RYR2_counts", "MUC12_counts"),
        ylab = "")

```

Write a few sentence making observations about your plot, noting the significance of what this means, and any key takeaways: RYR2 is more expressed than MUC12.
...
Does there appear to be a relationship between expression of your gene and five year survival? Is this what we would expect (do some research on the gene+survival to validate). Write a few sentence making observations about your plot, noting the significance of what this means, and any key takeaways: There is no significant difference between the expression of MUC12 and the patient's five year survival rate. 
...
geneABCD_counts <- data.frame(as.numeric(RYR2_counts[1,]),
                              as.numeric(MUC12_counts[1,]),
                              as.numeric(MKI67_counts[1,]),
                              as.numeric(PTGS2_counts[1,]))
...
cols[rna_clinical$age_category == "old"] <- "blue" # fill in column and value for as many unique values are in your column (ex: 2 for age_category, "young"/"old")
cols[rna_clinical$age_category == "young"] <- "red"

# run this line to create your plot!
pairs(geneABCD_counts, col = cols, lower.panel=NULL)
```

Do there appear to be any relationships between the genes in question? Do you see any clear clustering based on your chosen clinical variables? Write a few sentence making observations about your plot, noting the significance of what this means, and any key takeaways: There is no significant correlation between the expression of RYR2, MUC12, MKI67, and PTGS2.

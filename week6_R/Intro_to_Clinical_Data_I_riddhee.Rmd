...
```{r}
write.csv(clinic, "/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/riddheemehta/Desktop/Code/QBIO/qbio_490_riddhee/analysis_data/brca_clinical_data.csv")
```
...
THINK: what types of variables can you use in a scatter plot (continuous, discrete, categorical). Look these up if you need help! Categorical & discrete

For an extra challenge, add a plot title, axes titles, and colors to your plot.

```{r Exercise 4.1}
plot(clinic$age_at_initial_pathologic_diagnosis, clinic$days_to_last_followup, main = "Age at Diagnosis vs Days to Last Followup", xlab = "Age at Diagnosis", ylab = "Days to Last Followup")
```

Choose two new variables and make a box plot (recall the boxplot() function).

THINK: what types of variables can you use in a box plot? One categorical & one continous or discrete

For an extra challenge, add a plot title, axes titles, and confidence intervals to your plot.

```{r Exercise 4.2}
boxplot(clinic$age_at_initial_pathologic_diagnosis ~ clinic$race_list, main = "Age at Diagnosis vs Race", xlab = "Race", ylab = "Age at Diagnosis")
```
...
```{r}
vital_status_na_mask <- ifelse(is.na(clinic$vital_status), F, T)
```
